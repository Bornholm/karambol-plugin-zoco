...
class FetchBoampCommand extends Command
{
  ...
  protected function execute(InputInterface $input, OutputInterface $output)
  {
    ...
    if(!$ftpPassword) {
      $output->writeln(sprintf('<error>Couldn\'t connect to "%s".</error>', $ftpServer));
      return 1;
    }
    ...
    if(!$loginResult) {
      $output->writeln(sprintf('<error>Couldn\'t login as "%s".</error>', $ftpUser));
      return 1;
    }
    ...
    foreach($remoteFiles as $remoteFilename) {
      ...
      if(!file_exists($destFile)) {
        ...
        ftp_get($ftpConn, $destFile, $remoteFile, FTP_BINARY);
      } else {
